---
publishDate: 2024-12-23T10:20:05
title: 善用6個Clean Code技巧寫Vue.js
excerpt: 在當今的前端開發領域，Vue.js 是極受歡迎的框架之一。其簡潔的語法和強大功能使得開發者能夠快速構建可維護、可擴展的應用。然而，隨著項目規模擴大，維護性和可讀性常常成為挑戰。此時，清晰、簡潔、易讀的代碼結構就顯得尤為重要。這篇文章將分享幾個運用「Clean Code」的技巧來提升Vue.js開發質量。
...
**重點項：** - 文件名應以 use 開頭，例如 `useContentWidthControl.ts` - Composable函式約定用PascalCase命名，並以"use"作為開頭，例如 `useLinkList()` - 一定要於`setup()`或`<script setup> `中使用，否則程式會報錯誤，就算Composable的函式還有包其它的Composable函式，也務必於function內進行引用。
...
<template>
  <div
    ...
    :class="floorDataList.list.length === 1 && floorDataList.list[0].isFavorite ? 'border-b-black' : ''"
  >
    <ToggleBtn />
  </div>
</template>
...
const bottomBorderStyle = computed(() => (data.list.length === 1 && data.list[0].isFavorite ? 'border-b-black' : ''));
</script>
